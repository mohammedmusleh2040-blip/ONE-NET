...
export default function Dashboard() {
  // ... (State and Loaders remain the same) ...
  ...
  // ===== Loaders (Unchanged, for brevity in display) =====
...
async function loadInvoicesRange(fromISO, toISO) {
  ...
  // ===== Computed (Unchanged, for brevity in display) =====
  ...
  const sales = useMemo(() => {
    ...
    const openingDebts = (customers || []).reduce((acc, c) => acc + Math.max(0, safeNum(c.opening_balance)), 0);
    return invDebts + openingDebts;
  }, [invoices, customers]);

  const debts = useMemo(() => {
    const invDebts = (invoices || []).reduce((acc, inv) => {
      const remaining = pick(inv, ["remaining_amount"], null);
      if (remaining !== null) return acc + Math.max(0, safeNum(remaining));
      const tot = safeNum(pick(inv, ["total_after_discount", "grand_total", "total"], 0));
      const paid = safeNum(pick(inv, ["paid_amount"], 0));
...
const incomeOtherTotal = useMemo(() => {
  ...
  const approxProfit = useMemo(() => {
    // تقريب: إجمالي المبيعات - مصروفات
    return sales + incomeOtherTotal - expenseOutTotal;
  }, [sales, incomeOtherTotal, expenseOutTotal]);

  const financialSummary = useMemo(() => {
  // cash = cash payments from customers + cash income - cash expenses (if method exists)
  ...
  return {
    cash: cashPayments + cashIncome - cashExpense,
    income: paymentsTotal + incomeOtherTotal,
    expenses: expenseOutTotal,
    debts,
    net: netCash,
  };
}, [cashPayments, paymentsTotal, incomeOtherTotal, expenseOutTotal, debts, netCash, expenses]);
  ...
  const moveRows = useMemo(() => {
    // **NEW: Limiting to 7 rows for shorter table**
  ...
  // ====================================================================
  // NEW: Updated JSX for better styling using .card and CSS variables
  // ====================================================================
  
  return (
    // relying on body and .page/main css for background and text color
    <div className="page" style={{ padding: '20px' }}>
      ...
      {/* ===== KPI Cards (6) - Elevated Style using grid-3 on small screens ===== */}
      ...
      {/* ===== Financial Summary - Compacted and Enhanced ===== */}
      <div className="card" style={{ ...customStyles.cardSection }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 20, flexWrap: "wrap" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: 'repeat(auto-fit, minmax(100px, 1fr))', gap: 12, flexGrow: 1 }}>
            {/* Card element using inline style for background to highlight positive/negative */}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            <table className="table">
              <thead>
                <tr>
                  <th>#</th>
                  <th>الكرت</th>
                  <th style={{ textAlign: 'center' }}>الرصيد الفعلي</th> {/* NEW: Renamed column */}
              ...
              <tbody>
                {stockDist.length === 0 ? (
                  ...
                  stockDist.slice(0, 25).map((x, idx) => {
                    const isLow = x.stock <= lowStockThreshold; // تنبيه رصيد منخفض
                    const stockColor = isLow ? 'var(--danger)' : (x.stock > 0 ? 'var(--accent)' : 'var(--muted)');
                    
                    return (
                      <tr key={x.id ?? idx} style={{ backgroundColor: isLow ? 'rgba(239, 68, 68, 0.1)' : 'transparent' }}> {/* NEW: Alert background */}
                        <td>{idx + 1}</td>
                        <td style={{ fontWeight: 600 }}>{x.name}</td>
                        <td style={{ fontWeight: 900, textAlign: 'center', color: stockColor }}>
                          {qty(x.stock)}
                          {isLow && <span style={{fontSize: 10, display: 'block', color: stockColor}}>(ينفد قريباً)</span>} {/* NEW: Alert text */}
                        </td>
        ...
        <div className="card" style={{ padding: '16px' }}>
            ...
            <table className="table">
              <thead>
                <tr>
                  ...
                  <th style={{ textAlign: 'center' }}>الرصيد بعد</th>
                </tr>
              </thead>
              <tbody>
                {moveRows.length === 0 ? (
                  ...
                  moveRows.map((r, idx) => (
                    <tr key={r.id ?? idx}>
                      ...
                      <td style={{ textAlign: 'center' }}>
                        <span style={customStyles.movementBadge(r.type)}>
                          {r.type}
                        </span>
                      </td>
                      <td>{r.card}</td>
                      <td style={{ fontWeight: 700, textAlign: 'center' }}>{qty(r.qty)}</td>
                      <td style={{ textAlign: 'center', color: 'var(--muted)' }}>{r.after === "" ? "-" : qty(r.after)}</td>
                    </tr>
                  ))
                )}
              </tbody>
          ...
          {loading && <div style={{ padding: 10, color: 'var(--muted)', fontSize: 12, textAlign: 'center' }}>جارٍ التحميل...</div>}
        </div>
      </div>
	    </div>
  );
