...
const incomeOtherTotal = useMemo(() => {
        ...
        <div className="card" style={{ padding: '16px' }}>
          ...
          <div className="table-wrap" style={{ overflowX: 'auto', maxHeight: 260, overflowY: 'auto' }}>
            <table className="table">
              <thead>
                <tr><th>#</th><th>الكرت</th><th style={{ textAlign: 'center' }}>الرصيد الفعلي</th>{/* NEW: Renamed column */}<th style={{ textAlign: 'center' }}>السعر</th></tr>
              </thead>
              <tbody>
                {stockDist.length === 0 ? (
  ...
  return (
    <tr
      key={x.id ?? idx}
      style={{ backgroundColor: isZero ? "rgba(239, 68, 68, 0.10)" : isLow ? "rgba(245, 158, 11, 0.10)" : "transparent" }}
    >
      <td>{idx + 1}</td>
            ...
            <table className="table">
              ...
              <tbody>
                {moveRows.length === 0 ? (
                  ...
                  moveRows.map((r, idx) => (
                    <tr key={r.id ?? idx}><td>{idx + 1}</td><td>{fmtDateTime(r.created_at)}</td><td style={{ textAlign: 'center' }}>
                        <span style={customStyles.movementBadge(r.type)}>
                          {r.type}
                        </span>
                      </td><td>{r.card}</td><td style={{ fontWeight: 700, textAlign: 'center' }}>{qty(r.qty)}</td><td style={{ textAlign: 'center', color: 'var(--muted)' }}>{r.after === "" ? "-" : qty(r.after)}</td></tr>
                  ))
                )}
              </tbody>
            </table>
