...
export default function Payments() {
  ...
  async function savePayment(e) {
    e?.preventDefault?.();
    if (!fCustomerId) return alert("اختر العميل");
    const amt = safeNum(fAmount);
    if (!Number.isFinite(amt) || amt === 0) return alert("اكتب مبلغ صحيح (غير صفر)");

    setSaving(true);
    try {
      const payload = {
        customer_id: fCustomerId,
        invoice_id: fInvoiceId || null,
        amount: amt, // ✅ موجب=قبض / سالب=صرف
        method: fMethod,
        note: fNote || null,
      ...
      console.error(e2);
      alert(e2?.message || "فشل حفظ السند");
    } finally {
      setSaving(false);
    }
  }
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((r, idx) => {
                  ...
                  return (
                    <tr key={r.id}>
                      <td>{idx + 1}</td>
                      <td>{new Date(r.created_at || Date.now()).toLocaleString("ar-EG")}</td>
                      <td>
                      ...
                      <td>{invLabel}</td>
                      <td>{r.method || "-"}</td>
                      <td style={{ fontWeight: 800 }}>{money(Math.abs(r.amount))}</td>
