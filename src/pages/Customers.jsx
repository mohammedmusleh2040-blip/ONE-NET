      ...
      <div className="card">
        ...
        <div style={{ overflowX: "auto" }}>
          <table className="table">
            <thead>
              <tr><th>#</th><th>الاسم</th><th>النوع</th><th>الهاتف</th><th>افتتاحي</th><th>إجراءات</th></tr>
            </thead>
            <tbody>
              {filtered.map((r) => (
                <tr key={r.id}><td>{r.id}</td><td>{r.name}</td><td>{r.type === "giga" ? "جيجا" : "كروت"}</td><td>{r.phone || "-"}</td><td>{Number(r.opening_balance || 0).toFixed(2)}</td><td style={{ whiteSpace: "nowrap" }}>
