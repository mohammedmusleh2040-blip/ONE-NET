      ...
      .map((r, idx) => {
        ...
        return `
          <tr>
            ...
            <td style="font-weight:800">${escapeHtml(r.qty)}</td>
            <td>${escapeHtml(r.before_qty ?? "")}</td>
            <td>${escapeHtml(r.after_qty ?? "")}</td>
      ...
      .map((r, idx) => {
        return `
          <tr>
            <td>${idx + 1}</td>
            <td>${escapeHtml(r.card_name)}</td>
            <td>${escapeHtml(r.card_type_id)}</td>
            <td style="font-weight:800">${escapeHtml(money(r.balance_now))}</td>
            <td>${escapeHtml(money(r.inQty))}</td>
            <td>${escapeHtml(money(r.outQty))}</td>
            <td>${escapeHtml(money(r.net))}</td>
            <td>${escapeHtml(r.moves)}</td>
          </tr>
        `;
      })
      .join("");
  ...
  async function printCustomerLedger() {
    ...
    const htmlRows = rows
      .map((r, idx) => {
        ...
        return `
          <tr style="${isDetail ? "opacity:0.85;" : ""}">
            <td>${idx + 1}</td>
            <td>${escapeHtml(fmtDate(r.created_at))}</td>
            <td>${escapeHtml(r.kind)}</td>
            <td>${escapeHtml(r.ref || "-")}</td>
            <td>${isDetail ? "" : escapeHtml(money(r.debit))}</td>
            <td>${isDetail ? "" : escapeHtml(money(r.credit))}</td>
                    ...
                    (filteredMoves || []).map((r, idx) => {
                      ...
                      return (
                        <tr key={r.id}>
                          ...
                          <td style={{ fontWeight: 900 }}>{r.qty}</td>
                          <td>{r.before_qty}</td>
                          <td>{r.after_qty}</td>
                          <td style={{ maxWidth: 320, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{r.note || "-"}</td>
                        </tr>
                      );
                    })
                  )}
                  ...
                  {(itemSummaryRows || []).length === 0 ? (
                    ...
                    (itemSummaryRows || []).map((r, idx) => (
                      <tr key={String(r.card_type_id) + "_" + idx}>
                        <td>{idx + 1}</td>
                        <td>{r.card_name}</td>
                        <td>{r.card_type_id}</td>
                        <td style={{ fontWeight: 900 }}>{money(r.balance_now)}</td>
                        <td>{money(r.inQty)}</td>
                        <td>{money(r.outQty)}</td>
                        <td>{money(r.net)}</td>
                        <td>{r.moves}</td>
                      </tr>
                    ))
                  ...
                  {!custId ? (
                    ...
                    (ledgerFiltered || []).map((r, idx) => (
                      <tr key={idx} style={r.is_detail ? { opacity: 0.85 } : undefined}>
                        <td>{idx + 1}</td>
                        <td>{fmtDate(r.created_at)}</td>
                        <td style={r.is_detail ? { color: "var(--muted)" } : undefined}>{r.kind}</td>
                        <td>{r.ref || "-"}</td>
                        <td>{r.is_detail ? "" : money(r.debit)}</td>
                        <td>{r.is_detail ? "" : money(r.credit)}</td>
                        <td>{r.is_detail ? "" : money(r.running)}</td>
                        <td style={{ maxWidth: 420, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{r.note || "-"}</td>
                      </tr>
                    ))
                  )}
                </tbody>
